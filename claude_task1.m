...
[t, X] = ode45(@(t,x) state_space(t, x, tau, p), [0 50], x0, opts);
...
% End-effector marker
h_ee = plot(ax, NaN, NaN, 'p', 'MarkerSize',16, ...
...
for i = 1:step:n
    ...
    trail_x(end+1) = jts(1,4);                             %#ok<SAGROW>
    trail_y(end+1) = jts(2,4);                             %#ok<SAGROW>
...
%
    ...
    % Solve M*q'' = tau - C*q' - G  for q'' (backslash = LU solve)
    ddq = M \ (tau - C*dq - G);
    dx  = [dq; ddq];
end
